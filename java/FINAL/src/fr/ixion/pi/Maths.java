...
public class Maths {
	...
	public static String angD(double D) {
		String str = (int) floor(D) + "°" + (int) floor((D - floor(D)) * 60) + "'"
				+ ((((D - floor(D)) * 60) - ((int) floor((D - floor(D)) * 60))) * 60) + "''";
		Utils.log(str);
		return str;
	}

	public static double mod2pi(double x) {
		double bo = x / (2 * Math.PI);
		double ao = (2 * Math.PI) * (bo - abs_floor(bo));
		if (ao < 0)
			ao = (2 * Math.PI) + ao;
		return ao;
	}

	public static double abs_floor(double x) {
		double ro;
		if (x >= 0.0) {
			ro = Math.floor(x);
		} else {
